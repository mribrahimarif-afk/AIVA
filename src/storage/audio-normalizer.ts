...
/**
 * Checks if a WAV buffer is already exactly 16kHz, 16-bit signed mono PCM.
 */
export function isExactAzureWav(buffer: Buffer): boolean {
  if (
    !buffer ||
    buffer.length < 44 ||
...
/**
 * Finds the ffprobe binary path from config or system PATH.
 */
function getFfprobePath(): string {
  const env = getEnv();
  return env.AIVA_FFPROBE_PATH && env.AIVA_FFPROBE_PATH.trim().length > 0
    ? env.AIVA_FFPROBE_PATH.trim()
    : "ffprobe";
}

/**
 * Probes the duration of an audio buffer or file in milliseconds.
 * Returns null if duration cannot be established.
 */
export async function probeAudioDurationMs(
  ...
  // 2. If an absolute file path is given, probe with ffprobe using safe arguments and timeout
  if (absolutePath) {
    try {
...
export async function normalizeAudioForAzure(
  ...
  // If source is ALREADY proven to be exact 16kHz mono 16-bit PCM WAV, copy it directly
  if (sourceBuffer && isExactAzureWav(sourceBuffer)) {
  ...
  try {
    // Convert to exact 16kHz, 1-channel, 16-bit signed PCM WAV
    await execFileAsync(
      ffmpegBin,
      [
        "-y",
    ...
    // Verify converted file exists and has valid header
    const convertedBuffer = await fs.promises.readFile(tempWavPath);
    if (!isExactAzureWav(convertedBuffer)) {
      throw new StorageError("Audio normalization produced invalid or non-16kHz-mono WAV output");
    }
    ...
    throw new StorageError(
      "Failed to normalize audio for Azure Speech recognition. Azure requires 16kHz 16-bit mono PCM WAV.",
      { cause: err }
    );
  }
}
